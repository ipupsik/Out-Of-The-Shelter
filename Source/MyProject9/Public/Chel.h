...
public:
	...
	void AddTargetArrow(AActor* TargetObj); //добавл€ет стрелку-подсказку на экран(стрелка прикрепл€етс€ к определенному объекту)
	void RemoveTargetArrow(AActor* TargetObj); //убирает стрелку-подсказку на экране(стрелка убираетс€ по определенному объекту)
...
public:
	...
	TArray<AActor*> TargetItems; //массив с предметами, к которым прив€заны стрелки-подсказки
